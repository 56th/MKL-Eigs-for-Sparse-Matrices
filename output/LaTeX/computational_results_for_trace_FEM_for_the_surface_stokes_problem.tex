...
Thus we have~$\int_\Gamma p \diff{\vect x} = 0$, $p \equiv p^e$, $\vect u \equiv \vect u^e$ in $\mathcal O(\Gamma)$, and $\vect u$ is a tangential field. % Note that $\vect u \ne \vect u^e$. (I do not know why whoever implemented this test did not choose $\vect u(\vect x) \coloneqq \vect P(\vect x)\,\tilde{\vect u}^e(\vect x)$ instead, but this is how it is. I leave this choice since I have a lot of tables already computed with~\eqref{exact_soln_2}.)
...
\begin{figure}[H]
	...
	%	\begin{subfigure}{.5\linewidth}
	%		\centering
	%		\includegraphicsw[1.]{m_plot_old.png}
	%	\end{subfigure}%
	\begin{subfigure}{.5\linewidth}
		\centering
	...
	\begin{subfigure}{.4\linewidth}
		$\vect m_{1/2} \coloneqq \{2, \textcolor{Gray}{4}, 4, 6, 8, 12, 18, 24\}$, 
		
		$\vect m_{1} \coloneqq \{\textcolor{Gray}{0}, 2, 4, 8, 16, 32, 64, 128\}$.
		\vskip .3cm
		We skip level~$h = 4.2\times 10^{-1}$ for~$\vect m_{1/2}$ and level~$h = 8.3\times 10^{-1}$ for~$\vect m_1$ so that $m \in \vect m_{1/2}$ or $\vect m_1$ behaves proportionally to~$h^{-1/2}$ and~$h^{-1}$, respectively
	\end{subfigure}%
...
\begin{table}[H]
	\centering\footnotesize
	\caption{Errors for normals and shape operator. Note that $\vect n_{\Gamma^2_h} = \vect n_\Gamma^e$ for our choice of~$\phi$ in~\eqref{exact_soln}, and the error of normals is zero (as expected). Note also that $\vect n_{\Gamma^{2\to 1}_{h/m}} = \vect n_{\Gamma^1_{h/m}}$ for our choice of~$\phi$, and $\vect n_{\Gamma^1_{h/m}}$ converges to $\vect n_\Gamma$ as one increases~$m$ for fixed~$h$}
...
\begin{table}[H]
	...
	\begin{subtable}{1.\linewidth}\centering
		\begin{tabular}[1.3]{|c||c|c||c|c||c||c|}
			...
			\hline
			\input{sphere_2_P2P1_patchnormals=1_shift=0.0h_form=inconsistent_m=1_vstab=1_conv_table.tex}
		\end{tabular}
	\end{subtable}
	\vskip 2mm	
	\begin{subtable}{1.\linewidth}\centering
		\begin{tabular}[1.3]{|c||c|c||c||c|}
			...
			\hline
			\input{sphere_2_P2P1_patchnormals=1_shift=0.0h_form=inconsistent_m=1_vstab=1_iters_table.tex}
		\end{tabular}
	\end{subtable}
\end{table}
...
\begin{table}[h!]
	...
	\begin{subtable}{1.\linewidth}\centering
		\begin{tabular}[1.3]{|c||c|c||c|c||c||c||c|c|c|}
			\hline
			\multicolumn{10}{|c|}{$m \in \vect m_{1/2}$ as in Figure~\ref{fig:m}} \\
			\hline
			...
			\hline
			\input{sphere_2_P2P1_normals=exact_shift=0.0h_form=consistent_m=12_vstab=-1.000000_conv_table.tex}
		\end{tabular}
	\end{subtable}
	\vskip 2mm	
	\begin{subtable}{1.\linewidth}\centering
		\begin{tabular}[1.3]{|c||c|c||c|c||c||c|}
			...
			$h$ & $\| \vect u_h\cdot\vect n \|_{\LTwo}$ & Order & $\| \vect P\,\vect u_h - \vect u \|_{\LTwo}$ & Order & Outer iterations & Residual norm \\
			\hline
			\input{sphere_2_P2P1_normals=exact_shift=0.0h_form=consistent_m=12_vstab=-1.000000_iters_table.tex}
		\end{tabular}
	\end{subtable}
	\vskip 4mm	
	\begin{subtable}{1.\linewidth}\centering

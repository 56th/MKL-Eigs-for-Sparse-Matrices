...
\title{Some computational results for trace finite element method for the surface (Navier--)Stokes problem}
\author{
...
\begin{align}\begin{split}\label{surfaceNS}
	...
	\Div_\Gamma \vect u &= -g, 
\end{split}\end{align}
cf.~\cite[p.~8]{Jankuhn_2018}. As standard in the analysis of the Navier--Stokes equations, we introduced dimensionless variables in~\eqref{surfaceNS} so that~$\|\vect u\|_{\LInfSpace} = 1$, with Reynolds number~$\nu^{-1}$. 
...
\subsection{Tangential component approximation}
...
\begin{align}\begin{split}\label{split:tang2}
	\frac{\partial\vect u_T}{\partial t} + \vect P\,(\vect u\cdot\nabla)\vect u_T - \nu\,\vect P\Div_\Gamma(E_s(\vect u_T)) + u_N\,\vect H\,\vect u_T + \nabla_\Gamma p &= \vect f_T,\\
...
\begin{align}\begin{split}\label{split:tang:oseen}
	\alpha\,\vect u_T^i + \vect P\,(\vect w\cdot\nabla)\vect u_T^i - \nu\,\vect P\Div_\Gamma(E_s(\vect u_T^i)) + \beta\,\vect H\,\vect u_T^i + \nabla_\Gamma p^i &= \hat{\vect f}_T,\\
...
\begin{itemize}
	...
	\begin{equation*}
		\alpha = \Delta t^{-1},\quad
		\vect w = \vect u^0,\quad
		\beta = u_N^1,\quad
	...
	\begin{equation*}
		\alpha = \frac 32\,\Delta t^{-1},\quad
		\vect w = 2\,\vect u^{i-1} - \vect u^{i-2},\quad
		\beta = u_N^i,\quad

...
\begin{align}\begin{split}\label{surfaceNS}
	\rho\,D\vect u - 2\,\mu\Div_\Gamma(E_s(\vect u)) + \nabla_\Gamma p - p\,\kappa\,\vect n &= \vect f,\\
	\Div_\Gamma \vect u &= -g, 
\end{split}\end{align}
cf.~\cite[p.~8]{Jankuhn_2018}. Performing operator splitting technique, one may rewrite system~\eqref{surfaceNS} as two systems: One for the normal, and another one for the tangential component of the velocity field $\vect u = \vect u_T + u_N\,\vect n$. Namely,
\begin{align}\begin{split}\label{split:tang}
	\rho\,\vect P D\vect u_T - 2\,\mu\,\vect P\Div_\Gamma(E_s(\vect u_T)) + \nabla_\Gamma p + \rho\,u_N\,\vect H\,\vect u_T &= \vect f_T + \vect P \Div_\Gamma(u_N\,\vect H) + \rho\,u_N\,\nabla_\Gamma u_N,\\
	\Div_\Gamma \vect u_T &= -g - u_N\,\kappa
\end{split}\end{align}
and 
\begin{equation}\label{split:norm}
	\rho\,D u_N = 2\,\mu\,\vect n\cdot\Div_\Gamma(E_s(\vect u)) + p\,\kappa + f_N + \rho\,D \vect n\cdot\vect u_T. 	
\end{equation}
Note that systems~\eqref{split:tang} and~\eqref{split:norm} are coupled. \AZ{TODO: rewrite~\eqref{split:norm} similar to~\eqref{split:tang}, i.e. in such a form so that the left-hand side \textbf{only} has terms dependent on~$u_N$, and all the rest goes to the right-hand side.}
...
\begin{equation}\label{split:ls}
	D \phi \coloneqq \frac{\partial \phi}{\partial t} + u_N\,\vect n\cdot\nabla\phi = 0.
\end{equation} 

\subsection{Tangential equation approximation}
...
\begin{align}\begin{split}\label{split:tang2}
	\rho\,\left(\frac{\partial\vect u_T}{\partial t} + \vect P\,(\vect u\cdot\nabla)\vect u_T\right) - 2\,\mu\,\vect P\Div_\Gamma(E_s(\vect u_T)) + \nabla_\Gamma p + \rho\,u_N\,\vect H\,\vect u_T &= \vect f_T,\\
	\Div_\Gamma \vect u_T &= -g,
\end{split}\end{align}
where for the sake of simplicity we kept the same notation for the given right-hand side data (i.e. we set~$g \leftarrow g + u_N\,\kappa$ and similar for~$\vect f_T$).

We start with the time discretization. Consider the time frame~$t_i \coloneqq i\,\Delta t$. Approximating the time derivative with some finite difference and linearizing the convective term via extrapolation of the wind field from previous time frames $t_{j<i}$, we arrive at the following Oseen-type problem posed on the stationary surface~$\Gamma(t_i)$: Find~$(\vect u_T^i,\,p^i)$ such that
\begin{align}\begin{split}\label{split:tang:oseen}
	\alpha\,\vect u_T^i + \vect P\,(\vect w\cdot\nabla)\vect u_T^i - 2\,\mu\,\vect P\Div_\Gamma(E_s(\vect u_T^i)) + \nabla_\Gamma p^i + \beta\,\vect H\,\vect u_T^i &= \hat{\vect f}_T,\\
	\Div_\Gamma \vect u_T^i &= -\hat g.
\end{split}\end{align}
\begin{itemize}
	\item At the first time frame $i = 1$ we utilize backward Euler approximation for the time derivative and initial data $\vect u(\cdot, 0) = \vect u^0$ for the wind field~$\vect w$. Thus we have
	\begin{equation*}
		\alpha = \rho\,\Delta t^{-1},\quad
		\vect w = \rho\,\vect u^0,\quad
		\beta = \rho\,u_N^1,\quad
		\hat{\vect f}_T = \vect f_T^1 + \Delta t^{-1}\,\vect u_T^0,\quad
		\hat g = g^1.
	\end{equation*}
	\item At time frames $i > 1$ we utilize BDF2 approximation for the time derivative and linear extrapolation for the wind field~$\vect w$. Thus we have
	\begin{equation*}
		\alpha = \rho\,\frac 32\,\Delta t^{-1},\quad
		\vect w = \rho\,(2\,\vect u^{i-1} - \vect u^{i-2}),\quad
		\beta = \rho\,u_N^i,\quad
		\hat{\vect f}_T = \vect f_T^i + 2\,\Delta t^{-1}\,\vect u_T^{i-1} - \frac 12\,\Delta t^{-1}\,\vect u_T^{i-2},\quad
		\hat g = g^i.
		\end{equation*}
	\end{itemize}
This results in a semi-implicit time scheme.

\subsection{Normal component approximation}
...
\begin{itemize}
	...
	\item Once I am done with implementing~\eqref{split:tang:oseen} (quite soon), I will test it on some smooth exact solution. Then I will test the Kelvin--Helmoltz problem on a torus. Here (\url{https://arxiv.org/pdf/1803.06893.pdf}, \url{https://arxiv.org/pdf/1909.06229.pdf}) Christoph uses ``smooth'' initial data. Is it essential? I was thinking about simply rotating the lower part of the torus, and keeping the upper part at rest (or moving it in the opposite direction), i.e. having ``jump'' discontinuity in the initial velocity. The moment equation is of parabolic type, so it shall smooth the solution anyway.
	\item For the Kelvin--Helmoltz problem we want convection to dominate, but is our solver ready for this? Especially for not that small~$\Delta t$. Do not we need something like AL preconditioner here?    
\end{itemize}
}
...
\begin{table}[H]
	...
	\begin{subtable}{1.\linewidth}\centering
		\begin{tabular}[1.2]{|c||c|c||c|c||c||c|}
			...
			$h$ & $\|\vect u - \vect u_h\|_{\HOne}$ & Order & $\|\vect u - \vect u_h\|_{\LTwo}$ & Order & $\|p - p_h\|_{\LTwo}$ & Order \\
			\hline
			\input{sphere_2_P2P1_patchnormals=0_shift=0.0_form=consistent_m=12_stab=full_rhopfac=1_rhoppow=1_rhoufac=1_rhoupow=-1_conv_table.tex}
		\end{tabular}
	\end{subtable}
\end{table}	
\vskip -.6cm
\begin{table}[H]
	...
	\begin{subtable}{1.\linewidth}\centering
		\begin{tabular}[1.2]{|c||c|c||c||c|}
			...
			$h$ & $\| \vect u_h\cdot\vect n \|_{\LTwo}$ & Order & Outer iterations & Residual norm \\
			\hline
			\input{sphere_2_P2P1_patchnormals=0_shift=0.0_form=consistent_m=12_stab=full_rhopfac=1_rhoppow=1_rhoufac=1_rhoupow=-1_iters_table.tex}
		\end{tabular}
	\end{subtable}
\end{table}
\addtocounter{table}{-1}
...
\begin{table}[H]
	...
	\begin{tabular}[1.2]{|c||c|c||c|c||c||c|}
		...
		$h$ & $\|\vect u - \vect u_h\|_{\HOne}$ & Order & $\|\vect u - \vect u_h\|_{\LTwo}$ & Order & $\|p - p_h\|_{\LTwo}$ & Order \\
		\hline
		\input{sphere_2_P2P1_patchnormals=0_shift=0.0_form=consistent_m=1_rhopfac=1_rhoppow=1_rhoufac=1_rhoupow=-1_conv_table.tex}
	\end{tabular}
\end{subtable}
\end{table}	
\vskip -.6cm
...
\begin{subtable}{1.\linewidth}\centering
	\begin{tabular}[1.2]{|c||c|c||c||c|}
		...
		$h$ & $\| \vect u_h\cdot\vect n \|_{\LTwo}$ & Order & Outer iterations & Residual norm \\
		\hline
		\input{sphere_2_P2P1_patchnormals=0_shift=0.0_form=consistent_m=1_rhopfac=1_rhoppow=1_rhoufac=1_rhoupow=-1_iters_table.tex}
	\end{tabular}
\end{subtable}
\end{table}
\addtocounter{table}{-1}
...
\begin{table}[H]
	...
	\begin{subtable}{1.\linewidth}\centering
		\begin{tabular}[1.2]{|c||c|c||c|c||c||c|}
			...
			$h$ & $\|\vect u - \vect u_h\|_{\HOne}$ & Order & $\|\vect u - \vect u_h\|_{\LTwo}$ & Order & $\|p - p_h\|_{\LTwo}$ & Order \\
			\hline
			\input{sphere_2_P2P1_patchnormals=0_shift=0.0_form=consistent_m=12_rhopfac=1_rhoppow=0_rhoufac=1_rhoupow=-1_conv_table.tex}
		\end{tabular}
	\end{subtable}
\end{table}	
\vskip -.6cm
\begin{table}[H]
	...
	\begin{subtable}{1.\linewidth}\centering
		\begin{tabular}[1.2]{|c||c|c||c||c|}
			...
			$h$ & $\| \vect u_h\cdot\vect n \|_{\LTwo}$ & Order & Outer iterations & Residual norm \\
			\hline
			\input{sphere_2_P2P1_patchnormals=0_shift=0.0_form=consistent_m=12_rhopfac=1_rhoppow=0_rhoufac=1_rhoupow=-1_iters_table.tex}
		\end{tabular}
	\end{subtable}
\end{table}
\addtocounter{table}{-1}
...
\begin{table}[H]
	...
	\begin{subtable}{1.\linewidth}\centering
		\begin{tabular}[1.2]{|c||c|c||c|c||c||c|}
			...
			$h$ & $\|\vect u - \vect u_h\|_{\HOne}$ & Order & $\|\vect u - \vect u_h\|_{\LTwo}$ & Order & $\|p - p_h\|_{\LTwo}$ & Order \\
			\hline
			\input{sphere_2_P2P1_patchnormals=0_shift=0.0_form=consistent_m=12_rhopfac=1_rhoppow=-1_rhoufac=1_rhoupow=-1_conv_table.tex}
		\end{tabular}
	\end{subtable}
\end{table}	
\vskip -.6cm
\begin{table}[H]
	...
	\begin{subtable}{1.\linewidth}\centering
		\begin{tabular}[1.2]{|c||c|c||c||c|}
			...
			$h$ & $\| \vect u_h\cdot\vect n \|_{\LTwo}$ & Order & Outer iterations & Residual norm \\
			\hline
			\input{sphere_2_P2P1_patchnormals=0_shift=0.0_form=consistent_m=12_rhopfac=1_rhoppow=-1_rhoufac=1_rhoupow=-1_iters_table.tex}
		\end{tabular}
	\end{subtable}
\end{table}
\addtocounter{table}{-1}
...
\begin{table}[H]
	...
	\begin{subtable}{1.\linewidth}
		...
		\begin{tabular}[1.2]{|c|c|c|c|c|c|c|}
			...
			\hline
			\input{sphere_2_P2P1_consistent_diff_table.tex}
		\end{tabular}
	\end{subtable}%
\end{table}  
...
\begin{table}[H]
	...
	\begin{subtable}{1.\linewidth}
		...
		\begin{tabular}[1.2]{|c|c|c|c|c|c|c|c|c|}
			...
			& & & $\lambda_2$ & $\lambda_{n_{\vect S}}$ & $\lambda_2$ & $\lambda_{n_{\vect S}}$ & $\lambda_2$ & $\lambda_{n_{\vect S}}$ \\ 
			\hline
			\input{torus_P2P1_consistent_table.tex}
		\end{tabular}
	\end{subtable}%
\end{table}
\addtocounter{table}{-1}
...
\begin{table}[H]
	...
	\begin{subtable}{1.\linewidth}
		...
		\begin{tabular}[1.2]{|c|c|c|c|c|c|c|}
			...
			& $\lambda_2$ & $\lambda_{n_{\vect S}}$ & $\lambda_2$ & $\lambda_{n_{\vect S}}$ & $\lambda_2$ & $\lambda_{n_{\vect S}}$ \\ 
			\hline
			\input{torus_P2P1_consistent_h=0.104167_table.tex}
		\end{tabular}
	\end{subtable}
\end{table}
\addtocounter{table}{-1}
...
%		\caption{$\sphere + \alpha\,\vect s$}
%	\end{subfigure}%
%	\caption{The unit sphere (left) and the shifted unit sphere (right). Here $\vect s = (0, 1, 1)^T/\sqrt{2}$, $\alpha = 0.2$, and $h = 2.08\times10^{-1}$. The bulk mesh~$\Omega_h^\Gamma$ is computed for~$\sphere$ and then used for~$\sphere + \alpha\,\vect s$}
%	\label{fig:shift}		
%\end{figure}
%
%\begin{table}[h]
